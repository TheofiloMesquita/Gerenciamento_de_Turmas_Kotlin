...
import com.example.agenda.adapter.SalaAdapter
import com.example.agenda.model.Sala
import com.example.agenda.repository.SalaRepository
import com.google.android.material.floatingactionbutton.FloatingActionButton

class SalaActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setToolbar("Salas", R.id.nav_sala)
        ...
        saveItemButton.setOnClickListener {
            ...
            if (nome.isNotBlank()) {
                SalaRepository.addSala(nome)
                adapter.updateData(SalaRepository.getSalas())
    ...
    private fun setupRecyclerView(recyclerView: RecyclerView) {
        adapter = SalaAdapter(
            SalaRepository.getSalas(),
            onUpdateClick = { sala -> showUpdateDialog(sala) },
            onDeleteClick = { sala -> showDeleteDialog(sala) }
        )
    ...
    private fun showUpdateDialog(sala: Sala) {
        val editText = EditText(this)
        editText.setText(sala.nome)

        AlertDialog.Builder(this)
            .setTitle("Atualizar Sala")
            .setView(editText)
            .setPositiveButton("Salvar") { _, _ ->
                val novoNome = editText.text.toString()
                if (novoNome.isNotBlank()) {
                    SalaRepository.updateSala(sala.id, novoNome)
                    adapter.updateData(SalaRepository.getSalas())
                    setUnsavedChanges(true)
                } else {
                    Toast.makeText(this, "O nome não pode estar vazio", Toast.LENGTH_SHORT).show()
    ...
    private fun showDeleteDialog(sala: Sala) {
        AlertDialog.Builder(this)
            .setTitle("Excluir Sala")
            .setMessage("Tem certeza de que deseja excluir '${sala.nome}'?")
            .setPositiveButton("Sim") { _, _ ->
                SalaRepository.deleteSala(sala.id)
                adapter.updateData(SalaRepository.getSalas())
                setUnsavedChanges(true)
            }
